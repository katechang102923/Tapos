...
export function MerchantDashboard() {
  return (
    <LoginGate allowedRoles={["merchant", "admin"]} title="早餐店後台登入">
      {({ profile, signOutUser }) => <MerchantDashboardContent role={profile?.role ?? "user"} storeId={profile?.storeId ?? ""} onSignOut={signOutUser} />}
    </LoginGate>
  );
}
